...
class Detect(nn.Module):
    ...
    def forward(
        ...
        if self.export and self.format == "rknn":
            y = []
            # exporter 会在 rknn 导出前设 end2end=False，但 YOLO26 仍须用 one2one 头（与 predict 一致）。
            has_one2one = hasattr(self, "one2one_cv2") and self.one2one_cv2 is not None and len(self.one2one_cv2) > 0
            box_heads = self.one2one_cv2 if has_one2one else self.cv2
            cls_heads = self.one2one_cv3 if has_one2one else self.cv3
            for i in range(self.nl):
                y.append(box_heads[i](x[i]))  # reg: [1, 4*reg_max, H, W]
                y.append(cls_heads[i](x[i]))  # cls: [1, nc, H, W]
            return y
...
class Pose(Detect):
    ...
    def postprocess(self, preds: torch.Tensor) -> torch.Tensor:
        ...
        Retu# Special handling for RKNN export
            if self.format == "rknn":
                return kpts
