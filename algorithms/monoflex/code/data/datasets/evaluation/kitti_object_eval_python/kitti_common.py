...
from ...kitti_utils import compute_obj_rotation_y_from_camera_euler_angle, convertRot2Alpha
...
def get_label_anno(label_path):
    ...
    annotations['bbox'] = np.array(
        [[float(info) for info in x[3:7]] for x in content]).reshape(-1, 4)
    # dimensions will convert hwl format to standard lhw(camera) format.
    annotations['dimensions'] = np.array(
        [[float(info) for info in x[7:10]] for x in content]).reshape(
            -1, 3)[:, [2, 0, 1]]
    annotations['location'] = np.array(
        [[float(info) for info in x[16:19]] for x in content]).reshape(-1, 3)

    for x in content:
        ry = compute_obj_rotation_y_from_camera_euler_angle(float(x[19]), float(x[20]) ,float(x[21]))
        annotations['rotation_y'].append(ry)
        annotations['alpha'].append(convertRot2Alpha(ry, float(x[18]), float(x[16])))
    annotations['rotation_y'] = np.array(annotations['rotation_y']).reshape(-1)
    annotations['alpha'] = np.array(annotations['alpha'])

    if len(content) != 0 and len(content[0]) == 23:  # have score
        annotations['score'] = np.array([float(x[22]) for x in content])
    else:
