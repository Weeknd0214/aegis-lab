...
class KITTIDataset(Dataset):
	def __init__(self, cfg, root, is_train=True, transforms=None, augment=True):
		...
		self.label_dir = os.path.join(root, "label_2") if is_train else os.path.join(root, "label_2_val")
		...
		self.ref_focal_length = cfg.INPUT.REF_FOCAL_LENGTH
	...
	def get_image(self, idx):
		...
		if not Path(img_filename).is_file():
			img_filename = Path(img_filename).with_suffix('.jpg')
	...
	def get_calibration(self, idx, ori_width, ori_height, use_right_cam=False):
		calib_filename = os.path.join(self.calib_dir, self.label_files[idx])
		return Calibration(calib_filename, ori_width, ori_height, self.input_width, self.input_height, self.ref_focal_length, use_right_cam=use_right_cam)

	def get_label_objects(self, idx, img_x_scale, img_y_scale, focal_factor, calib_V2C):
		if self.split != 'test':
			label_filename = os.path.join(self.label_dir, self.label_files[idx])
		return read_label(label_filename, img_x_scale, img_y_scale, focal_factor, calib_V2C)
	...
	def __getitem__(self, idx):
		...
		if idx >= self.num_samples:
			...
			objs = None if self.split == 'test' else self.get_label_objects(idx, calib.focal_factor, calib.V2C)
			...
			ori_width = img.size[0]   # 获取宽度
			ori_height = img.size[1]   # 获取高度
			calib = self.get_calibration(idx, ori_width, ori_height)
			img = img.resize((int(ori_width*calib.img_x_scale), int(ori_height*calib.img_y_scale)), Image.NEAREST)
			objs = None if self.split == 'test' else self.get_label_objects(idx, calib.img_x_scale, calib.img_y_scale, calib.focal_factor, calib.V2C)
		...
		corners3ds = np.zeros([self.max_objs, 8, 3], dtype=np.float32)
		...
		for i, obj in enumerate(objs):
			...
			if self.filter_annos:
				# if math.isclose(float_truncation, 1.0):
				if float_truncation > 0.9:
					# self.logger.info("complete float_truncation and obj.t = {}.".format(obj.t))
					continue
				if int(float_occlusion) == 2:
					# self.logger.info("complete occlusion and obj.t = {}.".format(obj.t))
					continue
				if float_truncation >= self.filter_params[0] and (box2d[2:] - box2d[:2]).min() <= self.filter_params[1]: continue
				if (box2d[2:] - box2d[:2]).min() <= 4:
					# self.logger.info("too small or none box2d and truncation = {}, obj.t = {}.".format(float_truncation, obj.t))
					continue
				# continue euler y angle 90
		...
		target.add_field("corners3ds", corners3ds)
