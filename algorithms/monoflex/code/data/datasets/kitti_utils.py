...
TYPE_CONVERSION = {
    ...
    'Animal': 'UNKNOWN_MOVABLE',
    'PushCart': 'UNKNOWN_MOVABLE',
    'UNKNOWN_MOVABLE': 'UNKNOWN_MOVABLE',
    'WarningPost': 'UNKNOWN_UNMOVABLE',
    'AntiCollisionBucket': 'UNKNOWN_UNMOVABLE',
    'TrafficBarrier': 'UNKNOWN_UNMOVABLE',
    'Debris': 'UNKNOWN_UNMOVABLE',
...
def compute_obj_rotation_xy_from_lidar_euler_angle(calib_V2C, rx, ry, rz):
    roat_x = np.array([
        [1,0,0],
    ...
    # roat = np.matmul(roat_x,np.matmul(roat_y,roat_z))
    roat_V = np.matmul(roat_x,np.matmul(roat_y, roat_z))
    roat_C = np.matmul(calib_V2C[:, 0:3], roat_V)
    point_start_y = np.matmul(roat_C, np.array([0.0, 0.0, 0.0]))
    point_end_y = np.matmul(roat_C, np.array([10.0, 0.0, 0.0]))
    rotation_y = -math.atan2(point_end_y[2] - point_start_y[2], point_end_y[0] - point_start_y[0])
    point_start_x = np.matmul(roat_C, np.array([0.0, 0.0, 0.0]))
    point_end_x = np.matmul(roat_C, np.array([0.0, 10.0, 0.0]))
    rotation_x = math.atan2(point_end_x[2] - point_start_x[2], point_end_x[1] - point_start_x[1])
    return rotation_x, rotation_y

def compute_obj_rotation_y_from_camera_euler_angle(rx, ry, rz):
    roat_z = np.array([
        [math.cos(rz), math.sin(rz), 0],
        [-math.sin(rz), math.cos(rz), 0],
        [0, 0, 1]
    ...
    # roat = np.matmul(roat_x,np.matmul(roat_y,roat_z))
    roat = np.matmul(roat_z,np.matmul(roat_y,roat_x))
    point_start = np.matmul(np.array([0.0, 0.0, 0.0]), roat)
    point_end = np.matmul(np.array([10.0, 0.0, 0.0]), roat)
    rotation_y = -math.atan2(point_end[2] - point_start[2], point_end[0] - point_start[0])
    return rotation_y

def compute_obj_rotation_y_from_camera_euler_angle2(rx, ry, rz):
    roat_x = np.array([
        [1,0,0],
        [0,math.cos(rx),-math.sin(rx)],
        [0,math.sin(rx),math.cos(rx)]
    ...
    # roat = np.matmul(roat_x,np.matmul(roat_y,roat_z))
    roat = np.matmul(roat_x,np.matmul(roat_y, roat_z))
    point_start = np.matmul(roat, np.array([0.0, 0.0, 0.0]))
    point_end = np.matmul(roat, np.array([10.0, 0.0, 0.0]))
    rotation_y = -math.atan2(point_end[2] - point_start[2], point_end[0] - point_start[0])
    return rotation_y


def compute_obj_rot_mat(rx, ry, rz):
    rot_x = np.array([
        [1,0,0],
        [0,math.cos(rx),-math.sin(rx)],
        [0,math.sin(rx),math.cos(rx)]
    ...
    fix_rot_x = np.array([ # to fix yunce labels camera angle bug
        [1,0,0],
    ...
    rot_mat = np.matmul(np.matmul(rot_x,np.matmul(rot_y, rot_z)),fix_rot_x)
    return rot_mat


def compute_obj_rotation_y_from_camera_rot_mat_with_focal_scale(rot_mat, focal_factor):
    point_start = np.matmul(rot_mat, np.array([0.0, 0.0, 0.0]))
    point_end = np.matmul(rot_mat, np.array([10.0, 0.0, 0.0]))
    ori_rotation_y = -math.atan2(point_end[2] - point_start[2], point_end[0] - point_start[0])
    rotation_y = -math.atan2((point_end[2] - point_start[2]) / focal_factor, point_end[0] - point_start[0])
    return ori_rotation_y, rotation_y


def compute_camera_pitch_from_lidar(rotation, focal_factor):
    point_start = np.matmul(rotation, np.array([0.0, 0.0, 0.0]))
    point_end = np.matmul(rotation, np.array([0.0, 0.0, 10.0]))
    # 上仰为正、下俯为负
    # notice the front and rear camera and so on, the compute is different
    ori_pitch = math.atan2(point_end[2] - point_start[2], abs(point_end[0] - point_start[0]))
    pitch = math.atan2(point_end[2] - point_start[2], abs((point_end[0] - point_start[0]) / focal_factor))
    return ori_pitch, pitch

def convert_location_from_lidar_to_camera(calib_V2C, px, py, pz):
    vector_loc = np.array([px, py, pz, 1])
    cvted = np.matmul(calib_V2C, vector_loc)
    return cvted

class Object3d(object):
    """ 3d object label """

    def __init__(self, label_file_line, img_x_scale, img_y_scale, focal_factor, calib_V2C):
        data = label_file_line.split(" ")
        data[1:] = [float(x) for x in data[1:]]
        # extract label, truncation, occlusion
        self.type = self.label_type_convert(data[0])  # 'Car', 'Pedestrian', ...
        ...
        self.xmin = data[3] * img_x_scale # left
        self.ymin = data[4] * img_y_scale # top
        self.xmax = data[5] * img_x_scale # right
        self.ymax = data[6] * img_y_scale # bottom
        self.box2d = np.array([self.xmin, self.ymin, self.xmax, self.ymax], dtype=np.float32)

        # check error label data
        if (self.truncation < 1 and (self.box2d[2:] - self.box2d[:2]).max() < 0.1):
            print("!!!self.truncation < 1 and (self.box2d[2:] - self.box2d[:2]).max() < 0.1, data = ",
        ...
        self.ori_t = np.array((float(data[16]), float(data[17]), float(data[18])), dtype=np.float32)
        self.t = np.array((float(data[16]), float(data[17]), float(data[18]) / focal_factor), dtype=np.float32)  # location (x,y,z) in camera coord.
        self.dis_to_cam = np.linalg.norm(self.t)

        # yaw angle (around Y-axis in camera coordinates) [-pi..pi]
        ori_rot_mat = compute_obj_rot_mat(float(data[19]), float(data[20]) ,float(data[21]))
        # Don't use eq_ry before modifiy RandomHorizontallyFlip augmentation
        self.ry, self.eq_ry = compute_obj_rotation_y_from_camera_rot_mat_with_focal_scale(ori_rot_mat, focal_factor)

        self.ray = math.atan2(self.t[0], self.t[2])
        self.alpha = convertRot2Alpha(self.ry, self.t[2], self.t[0])
        if self.type == "TrafficCone": # unify the TrafficCone alpha
            self.alpha = math.pi * 0.5
            self.ry = convertAlpha2Rot(self.alpha, self.t[2], self.t[0])
    ...
    def label_type_convert(self, in_type):
        return TYPE_CONVERSION[in_type]
    ...
    def generate_corners3d(self, focal_factor, cam_pitch):
        """
        ...
        # rot_mat = np.matmul(rot_x, rot_y)

        corners3d = np.vstack([x_corners, y_corners, z_corners])  # (3, 8)
        corners3d = np.matmul(rot_y, corners3d).T
        corners3d = corners3d + self.ori_t
        corners3d[:, 2] /= focal_factor
...
class Calibration(object):
    ...
    def __init__(self, calib_filepath, ori_width, ori_height, input_width, input_height, ref_focal_l, from_video=False, use_right_cam=False):
        if from_video:
        ...
        self.xy_f_ratio = (self.P[0, 0] / self.P[1, 1]).item()
        # if not math.isclose(self.xy_f_ratio, 1.0, rel_tol=0.006):
        #     pass
        eq_width = float(ori_width) / self.xy_f_ratio
        self.img_x_scale /= self.xy_f_ratio
        eq_height = float(ori_height)

        input_scale = min(float(input_width) / eq_width, float(input_height) / eq_height)
        self.img_x_scale *= input_scale
        self.img_y_scale *= input_scale

        self.P[0, 0] *= self.img_x_scale
        self.P[1, 1] *= self.img_y_scale
        self.P[0, 2] = (self.P[0, 2] - 0.5) * self.img_x_scale + 0.5
        self.P[1, 2] = (self.P[1, 2] - 0.5) * self.img_y_scale + 0.5
        self.P[0, 3] *= self.img_x_scale
        self.P[1, 3] *= self.img_y_scale

        self.focal_factor = self.P[1, 1] / ref_focal_l
        self.P[0, 0] /= self.focal_factor
        self.P[1, 1] = ref_focal_l
        ...
        self.ori_cam_pitch, self.cam_pitch = compute_camera_pitch_from_lidar(self.C2V[0:3, 0:3], self.focal_factor)
...
def read_label(label_filename, img_x_scale, img_y_scale, focal_factor, calib_V2C):
    # print("label_filename:", label_filename)
    lines = [line.rstrip() for line in open(label_filename)]
    objects = [Object3d(line, img_x_scale, img_y_scale, focal_factor, calib_V2C) for line in lines]
    return objects
...
def draw_projected_box3d(image, qs, color=None, cls=None, thickness=1, draw_orientation=False, draw_height_text=False,
    ...
    if draw_height_text:
        ...
        for i in range(height_pos.shape[0]):
            cv2.putText(image, 'h_{}'.format(i + 1), tuple(height_pos[i].astype(np.int32)), cv2.FONT_HERSHEY_SIMPLEX, 
                                height_text_size, (255, 0, 0), 1, cv2.LINE_AA)

        cv2.putText(image, 'h_c'.format(i + 1), tuple(height_pos_center.astype(np.int32)), cv2.FONT_HERSHEY_SIMPLEX, 
                            height_text_size, (255, 0, 0), 1, cv2.LINE_AA)
...
def init_bev_image(out_size=1280):
    bird_view = np.ones((out_size, out_size, 3), dtype=np.uint8) * 230

    return bird_view

def draw_bev_box3d(image, boxes3d, color=(255, 255, 255), cls=None, thickness=1, scores=None, text_lables=[], world_size=128, out_size=1280):
    if color is None:   
        if cls is not None:
            if cls == 'Truck':
                # color = (55, 67, 225)
...
def get_3d_dis(image, objs, calib):
    ...
    corners_3d = obj.generate_corners3d(calib.focal_factor, calib.ori_cam_pitch)
    corners_2d, _ = calib.project_rect_to_image(corners_3d)

    box2d = proj3d_to_2d(corners_2d).astype(np.int32) 
    ...
    centers_2d = corners_2d[center_idxs].mean(axis=1).astype(np.int32)
    # vectors
    ...
    # for k in range(3):
    #     ptEnd = tuple(surface_proj[k].astype(np.int32).tolist())
...
def show_image_with_boxes(image, cls_ids, target_center, box2d, corners_2d, reg_mask, offset_3D, down_ratio, pad_size, encoded_alphas, vis=True, index=[]):
    ...
    id_to_cls = list(cfg.DATASETS.DETECT_CLASSES)
    ...
    for idx in range(target_center.shape[0]):
        ...
        cv2.circle(img3, tuple(center_2d[idx].astype(np.int32)), 4, (255, 0, 0), -1)
        cv2.circle(img3, tuple(ori_target_center[idx].astype(np.int32)), 4, (0, 255, 255), -1)
...
def show_edge_heatmap(img, edge_heatmap, interp_edge_indices, output_size):
    ...
    interp_edge_indices_int = interp_edge_indices.round().astype(np.int32)
...
def show_heatmap(img, heat_map, classes=list(cfg.DATASETS.DETECT_CLASSES), index=None):
