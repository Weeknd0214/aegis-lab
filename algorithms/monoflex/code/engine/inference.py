...
from tools.sany_eval_bev import sy_eval_bev
from data.datasets.kitti_utils import TYPE_CONVERSION
...
def compute_on_dataset(model, data_loader, device, predict_folder, timer=None, vis=False, 
                        eval_score_iou=False, eval_depth=False, eval_trunc_recall=False, classes=None):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(data_loader)):
            ...
            focal_factor = calib.focal_factor.item()
            img_x_scale = calib.img_x_scale
            img_y_scale = calib.img_y_scale
            generate_kitti_3d_detection(output, predict_txt, img_x_scale, img_y_scale, focal_factor, classes)
...
def inference(
    ...
    dis_ious = compute_on_dataset(model, data_loader, device, predict_folder, 
                                inference_timer, vis, eval_score_iou, classes=classes)
    comm.synchronize()
    ...
    ret_tuple = sy_eval_bev(label_path=dataset.label_dir,
                result_path=predict_folder,
                label_split_file=dataset.imageset_txt,
                current_class=dataset.classes,
                type_conversion=TYPE_CONVERSION)
    return ret_tuple[0:2] # F1_score_all_bev, F1_score_all
