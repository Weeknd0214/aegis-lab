...
def do_eval(cfg, model, data_loaders_val, iteration):
	...
	F1_score_all_bev, F1_score_all = inference(
		model,
		data_loaders_val,
	...
	return F1_score_all_bev, F1_score_all
...
def do_train(
	...
	for data, iteration in zip(data_loader, range(start_iter, max_iter)):
		...
		# if False:
			...
			F1_score_all_bev, F1_score_all = do_eval(cfg, model, data_loaders_val, iteration)
			...
			if comm.get_rank() == 0:
				...
				writer.add_scalar("F1_score_bev/{}".format(default_depth_method), F1_score_all_bev, eval_iteration + 1)
				writer.add_scalar("F1_score/{}".format(default_depth_method), F1_score_all, eval_iteration + 1)

				# record the best model according to the AP_3D, Car, Moderate, IoU=0.7
				# important_key = '{}_3d_{:.2f}/moderate'.format('Truck', 0.7)
				# eval_mAP = float(result_dict[important_key])
				if F1_score_all_bev >= best_mAP:
					# save best mAP and corresponding iterations
					best_mAP = F1_score_all_bev
					best_iteration = iteration
					# best_result_str = result_str
					checkpointer.save("model_moderate_best_{}".format(default_depth_method), **arguments)

					if cfg.SOLVER.EVAL_AND_SAVE_EPOCH:
						logger.info('epoch = {}, best_F1_score_all_bev = {:.2f}, best_F1_score_all = {:.2f}, updating best checkpoint for depth {} \n'.format(cur_epoch, F1_score_all_bev, F1_score_all, default_depth_method))
					else:
						logger.info('iteration = {}, best_F1_score_all_bev {:.2f}, best_F1_score_all = {:.2f}, updating best checkpoint for depth {} \n'.format(iteration, F1_score_all_bev, F1_score_all, default_depth_method))
