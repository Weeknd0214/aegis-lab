...
from data.datasets.evaluation.kitti_object_eval_python.evaluate import recover_obj_ori_ry_from_fixed_focal, recover_obj_dims_from_fixed_focal
...
def box3d_to_corners(locs, dims, roty, focal_factor = 1.0, cam_pitch = 0.0):
	# 3d bbox template
	h, w, l = dims
	ori_ry = roty
	...
	# rotation matirx
	Ry = np.array([[np.cos(ori_ry), 0, np.sin(ori_ry)],
				  [0, 1, 0],
				  [-np.sin(ori_ry), 0, np.cos(ori_ry)]])
	rotx = -cam_pitch
	Rx = np.array([[1, 0, 0],
	...
	ori_locs = locs.copy()
	ori_locs[2] *= focal_factor

	ori_corners3d = np.vstack([x_corners, y_corners, z_corners])  # (3, 8)
	ori_corners3d = np.matmul(Rx, np.matmul(Ry, ori_corners3d)).T
	ori_corners3d = ori_corners3d + ori_locs

	return ori_corners3d
...
def show_image_with_boxes_test(image, output, target, visualize_preds):
	...
	for i in range(box2d.shape[0]):
		...
		ori_corners3d = box3d_to_corners(locs[i], dims[i], rotys[i], calib.focal_factor, calib.ori_cam_pitch)
		corners3d = ori_corners3d.copy()
		corners3d[:, 2] /= calib.focal_factor
		corners_2d, depth = calib.project_rect_to_image(corners3d)
		img3 = draw_projected_box3d(img3, corners_2d, color=pred_color)

		corners3d_lidar = calib.project_rect_to_velo(ori_corners3d)
		...
		# depth of four edges
		edge_depth = calib.f_v * dims[i, 0] / edge_height
		center_depth = calib.f_v * dims[i, 0] / center_height
...
def show_image_with_boxes(image, output, target, visualize_preds, vis_scores=None):
	...
	gt_corners3ds = target.get_field('corners3ds')[valid_mask]
	...
	for i in range(box2d.shape[0]):
		...
		# TODO: Adjust rotation_y to display
		# corners3d_lidar = calib.project_rect_to_velo(corners3d)
		img4 = draw_bev_box3d(img4, ori_corners3d[np.newaxis, :], thickness=2, color=pred_color, scores=None)

	# plot ground-truth
	for i in range(num_gt):
		img2.draw_box(box_coord=gt_boxes[i], edge_color='r')
		img2.draw_text(text='{}'.format(ID_TYPE_CONVERSION[gt_clses[i].item()]), position=(int(gt_boxes[i, 0]), int(gt_boxes[i, 1])))

		ori_corners3d = box3d_to_corners(gt_locs[i].numpy(), np.roll(gt_dims[i].numpy(), -1), gt_rotys[i].numpy(), calib.focal_factor, calib.ori_cam_pitch)
		corners3d = ori_corners3d.copy()
		corners3d[:, 2] /= calib.focal_factor
		corners_2d, depth = calib.project_rect_to_image(corners3d)
		...
		# TODO: Adjust rotation_y to display
		# corners3d_lidar = calib.project_rect_to_velo(corners3d)
		img4 = draw_bev_box3d(img4, ori_corners3d[np.newaxis, :], thickness=2, color=gt_color, scores=None)
	...
	plt.subplot(332)
	plt.imshow(img2); plt.title('2D boxes'); plt.axis('off')
	plt.subplot(333)
	plt.imshow(image); plt.title('raw img'); plt.axis('off')
	# plt.subplot(212)
	plt.subplot2grid((3,3), (1,0), rowspan=3, colspan=3)
