...
from .head.detector_head import bulid_head2
...
class KeypointDetector2(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.backbone = build_backbone(cfg)
        self.heads = bulid_head2(cfg, self.backbone.out_channels)

    def forward(self, images, targets=None):
        features = self.backbone(images)

        assert self.training == False, "Export onnx need be in eval mode!"
        x = self.heads(features, targets, True)
