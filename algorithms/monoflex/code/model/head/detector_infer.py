...
class PostProcessor(nn.Module):
	...
	def forward(self, predictions, targets, features=None, test=False, refine_module=None):
		...
		if self.keypoint_depth_with_uncertainty:
			...
			if torch.isinf(pred_keypoint_uncertainty).any():
				pred_keypoint_uncertainty = torch.where(torch.isinf(pred_keypoint_uncertainty),
				torch.full_like(pred_keypoint_uncertainty, 7.9716e+36), pred_keypoint_uncertainty)
				print("pred_keypoint_uncertainty has inf and modified it")
				print(pred_keypoint_uncertainty)
