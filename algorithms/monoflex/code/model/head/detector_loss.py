...
class Loss_Computation():
	...
	def prepare_predictions(self, targets_variables, predictions):
		...
		if mask_regression_2D.sum() == 0:
			print("valid_targets_bbox_points = ", valid_targets_bbox_points)
			print("target_bboxes_2D = ", target_bboxes_2D)
			print("target_regression_2D = ", target_regression_2D)
	...
	def __call__(self, predictions, targets):
		...
		if num_reg_2D > 0:
			...
			print("num_reg_2D is not > 0 and num_reg_2D = ", num_reg_2D)
			reg_2D_loss = torch.tensor(0.01).cuda()
			iou_2D = torch.tensor(0.99).cuda()
		...
		if num_reg_3D > 0:
			...
			if torch.isnan(offset_3D_loss):
				offset_3D_loss = torch.tensor(0.01).cuda()
				print("offset_3D_loss is nan and set to ", offset_3D_loss)
		...
		for v in loss_dict.values():
			if torch.isnan(v).sum() > 0:
				# pdb.set_trace()
				print("torch.isnan(v) is true and loss_dict = ", loss_dict)
			if torch.isinf(v).sum() > 0:
				# pdb.set_trace()
				print("torch.isinf(v) is true and loss_dict = ", loss_dict)
