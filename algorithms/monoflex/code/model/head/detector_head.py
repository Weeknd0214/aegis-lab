...
class Detect_Head(nn.Module):
    ...
    def forward(self, features, targets=None, test=False):
        ...
        x['cls'] = x['cls'].clamp(min=1e-4, max=1 - 1e-4)
...
class Detect_Head2(nn.Module):
    def __init__(self, cfg, in_channels):
        super(Detect_Head2, self).__init__()
        self.predictor = make_predictor(cfg, in_channels)
...
def bulid_head2(cfg, in_channels):

    return Detect_Head2(cfg, in_channels)