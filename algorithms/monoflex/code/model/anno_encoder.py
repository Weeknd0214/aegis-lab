...
class Anno_Encoder():
		...
		def decode_depth_from_keypoints(self, pred_offsets, pred_keypoints, pred_dimensions, calibs, avg_center=False):
			...
			if avg_center:
				...
				center_depth = calib.f_v * pred_height_3D.unsqueeze(-1) / (center_height.abs() * self.down_ratio * 2)
		...
		def decode_depth_from_keypoints_batch(self, pred_keypoints, pred_dimensions, calibs, batch_idxs=None):
			...
			for idx, gt_idx in enumerate(torch.unique(batch_idxs, sorted=True).tolist()):			
				...
				center_depth = calib.f_v * pred_height_3D[corr_pts_idx] / (F.relu(center_height[corr_pts_idx]) * self.down_ratio + self.EPS)
				corner_02_depth = calib.f_v * pred_height_3D[corr_pts_idx].unsqueeze(-1) / (F.relu(corner_02_height[corr_pts_idx]) * self.down_ratio + self.EPS)
				corner_13_depth = calib.f_v * pred_height_3D[corr_pts_idx].unsqueeze(-1) / (F.relu(corner_13_height[corr_pts_idx]) * self.down_ratio + self.EPS)
