...
ranges_GTs = [
    [-30.0, 100.0, -10.0, 10.0],  # truck,
    [-30.0, 80.0, -10.0, 10.0],  # car,
    [-30.0, 60.0, -10.0, 10.0],  # ped,
    [-30.0, 60.0, -10.0, 10.0],  # motocyclist,
    [-30.0, 100.0, -10.0, 10.0],  # tractor,
    [-30.0, 40.0, -10.0, 10.0],  # fence,
    [-30.0, 80.0, -10.0, 10.0],  # unknown,
    [-30.0, 40.0, -10.0, 10.0],  # trafficcone,
]


pre_range_extend_x = 3
pre_range_extend_y = 1.5

# ranges_GTs = [
#     [-35.0, 100.0, -20.96, 20.96],  # truck,
#     [-35.0, 100.0, -20.96, 20.96],  # car,
#     [-35.0, 100.0, -20.96, 20.96],  # ped,
#     [-35.0, 100.0, -20.96, 20.96],  # motocyclist,
#     [-35.0, 100.0, -20.96, 20.96],  # fence,
#     [-35.0, 100.0, -20.96, 20.96],  # unknown,
#     [-35.0, 100.0, -20.96, 20.96],  # trafficcone,
# ]

# ranges_GTs = [
#     [-40.0, 100.0, -20.0, 20.0],  # truck,
#     [-40.0, 80.0, -20.0, 20.0],  # car,
#     [-40.0, 60.0, -10.0, 10.0],  # ped,
#     [-40.0, 60.0, -10.0, 10.0],  # motocyclist,
#     [-40.0, 40.0, -10.0, 10.0],  # fence,
#     [-40.0, 80.0, -20.0, 20.0],  # unknown,
#     [-40.0, 40.0, -10.0, 10.0],  # trafficcone,
# ]
...
thr_center_dist_bev = [
    ...
    2.0,  # thr_iou_car,
    1.0,  # thr_iou_ped,
    1.0,  # thr_iou_motocyclist,
    2.0,  # thr_iou_tractor,
    2.0,  # thr_iou_fence,
    1.0,  # thr_iou_unknown,
    0.5,  # thr_iou_trafficcone,
]
...
thr_ious_bev = [
    ...
    0.1,  # thr_iou_car,
    0.1,  # thr_iou_ped,
    0.1,  # thr_iou_motocyclist,
    0.1,  # thr_iou_tractor,
    0.1,  # thr_iou_fence,
    0.1,  # thr_iou_unknown,
    0.1,  # thr_iou_trafficcone,
]
...
#     0.001,  # thr_iou_car,
#     0.001,  # thr_iou_ped,
#     0.001,  # thr_iou_motocyclist,
#     0.001,  # thr_iou_fence,
#     0.001,  # thr_iou_unknown,
...
lidar_x_ind = label_fields.index("obj_x")
lidar_y_ind = label_fields.index("obj_y")
length_ind = label_fields.index("obj_l")
width_ind = label_fields.index("obj_w")
height_ind = label_fields.index("obj_h")
angle_ind = label_fields.index("obj_angle")


def get_bev_box(line):
    new_box = []
    lidar_x = float(line[lidar_x_ind])
    lidar_y = float(line[lidar_y_ind])
    length = abs(float(line[length_ind]))
    height = abs(float(line[height_ind]))
    width = abs(float(line[width_ind]))
    ry = float(line[angle_ind])
    angle = np.rad2deg(ry)
    new_box.append(lidar_x)
    new_box.append(lidar_y)
...
def compute_match_num2(
    ...
    for i, gt_line in enumerate(single_gt_list):
        ...
        for j, pre_line in enumerate(single_pre_list):
            ...
            iou_score_bev = evaluate(gt_box_bev, pre_box_bev)  # 两者在BEV上进行iou计算
            score_np[i, j] = (center_dist, iou_score_bev, 0)
...
def get_gt_and_pre_list(
    ...
    with open(gt_name_path, 'r') as f:
        ...
        for txt_result in data:
            ...
            lidar_y = float(line[lidar_y_ind])
            if (
                (lidar_x < ranges_GTs[cls_ind][0])
                or (lidar_x > ranges_GTs[cls_ind][1])
                or (lidar_y < ranges_GTs[cls_ind][2])
                or (lidar_y > ranges_GTs[cls_ind][3])
            ):
                continue
    ...
    if os.path.exists(pre_filename_path):  # 如果存在gt对应的预测txt文件
        with open(pre_filename_path, 'r') as ff:
            ...
            for txt_result in data:
                ...
                lidar_y = float(line[lidar_y_ind])
                if (
                    (lidar_x < ranges_GTs[cls_ind][0] - pre_range_extend_x)
                    or (lidar_x > ranges_GTs[cls_ind][1] + pre_range_extend_x)
                    or (lidar_y < ranges_GTs[cls_ind][2] - pre_range_extend_y)
                    or (lidar_y > ranges_GTs[cls_ind][3] + pre_range_extend_y)
                ):
                    continue
...
def sy_eval_bev(
    ...
    total_dirs = os.listdir(label_path)
    for gt_filename in total_dirs:  # 以Gt文件为基准，遍历测试
...
def main():
    ...
    eval_data_dir = "../PointPillars_eval/"
    ...
    eval_visual(
        total_ver_dcrpt,
        [f1_bev_notype_list, rc_bev_notype_list, pc_bev_notype_list],
        [f1_bev_list, rc_bev_list, pc_bev_list],
        all_dsp_clses,
        dsp_cls_lt,
        cls_f1_bev_lt_lt,
        cls_rc_bev_lt_lt,
